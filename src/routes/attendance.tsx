  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Attendance — Workday" },
      {
        ...
        content: "Monthly calendar, leave balance and today's activity.",
      },
    ],
  }),
  component: () => (
            ...
            <div className="rounded-3xl bg-card p-5" style={{ boxShadow: "var(--shadow-card)" }}>
              ...
              <div className="mt-3 grid grid-cols-3 gap-3">
                {quickReq.map(({ icon: Icon, label, tint }) => (
                  <button
                    key={label}
                    className="flex flex-col items-center gap-2 rounded-2xl p-3 text-center transition hover:bg-accent"
                  >
                    <div className={`grid h-12 w-12 place-items-center rounded-full ${tint}`}>
                      <Icon className="h-5 w-5" />
                    </div>
              ...
              {timeline.map((e, i) => {
                ...
                return (
                  <li key={i} className="relative">
                    <span
                      className={`absolute -left-[35px] grid h-8 w-8 place-items-center rounded-full ${e.tint}`}
                    >
